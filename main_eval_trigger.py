...
def train(train_loader, trigger_set_loader, trigger_label, model, criterion, optimizer, epoch):
    ...
    start = time.time()
    for i, ((image, target), (trigger_image, trigger_target, trigger_index)) in enumerate(zip(train_loader, trigger_set_loader)):
        ...
        image = torch.concatenate([image, trigger_image], 0)
        target = torch.concatenate([target, trigger_target], 0)
