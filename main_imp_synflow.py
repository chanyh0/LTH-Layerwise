...
def main():
    ...
    for state in range(start_state, 6):
    ...
    for name, m in model.named_modules():
        if isinstance(m, nn.Conv2d):
            scores[name] = torch.clone(m.weight_orig.grad).detach().abs_()
            m.weight_orig.grad.data.zero_()

    # normalize score
    all_scores = torch.cat([torch.flatten(v) for v in scores.values()])
    threshold = torch.kthvalue(all_scores, int(len(all_scores) * args.rate))[0]
