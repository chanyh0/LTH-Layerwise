...
best_sa = 0
from pruning_utils_unprune import get_reverse_flatten_params_fun, flatten_params
def prune_taylor1_abs(model, mask_dict, args):
    ...
    for key, param in zip(mask_dict.keys(), result):
        param[mask_dict[key] == 0] = 0
        zeros += (mask_dict[key] == 0).sum().item()
...
def main():
    ...
    pruning_model(model, 0.0, conv1=False)
    for state in range(start_state, args.pruning_times):
        ...
        new_mask = prune_taylor1_abs(model, current_mask, args)
