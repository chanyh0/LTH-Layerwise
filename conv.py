...
class SparseConv2D(torch.nn.Module):
  ...
  def __init__(self, in_channels: int, out_channels: int, kernel_size: _size_2_t, stride: _size_2_t  = 1, padding: _size_2_t = 1, dilation: _size_2_t = 1, bias: bool = True):
  ...
  def load(self, sparse_weight, bias):
    ...
    cols = self.extract_dense(sparse_weight)
    kernel_ptr.append(len(kernel_offset))

    for r in range(self.out_channels):
      kernel_offset.extend(cols)
      kernel_value.extend(sparse_weight[r,cols].tolist())
      kernel_ptr.append(len(kernel_offset))
      kernel_map.append(r)
      for c in cols:
    ...
    self.kernel_value = torch.nn.Parameter(torch.FloatTensor(kernel_value))
  ...
  def forward(self, input: Tensor) -> Tensor:  # input: HWCN
    ...
    if self.filename == None:
      ...
      if len(self.kernel_ptr_sparse) > 1:
    ...
    _lib = ctypes.CDLL(os.path.join(_libdir, self.filename+'.so'))
    ...
    _lib.spmm_conv(ctypes.c_void_p(input.data_ptr()), ctypes.c_void_p(output.data_ptr()), ctypes.c_void_p(self.kernel_ptr.data_ptr()), ctypes.c_void_p(self.kernel_map.data_ptr()),  ctypes.c_void_p(self.kernel_offset.data_ptr()), ctypes.c_void_p(self.kernel_value.data.data_ptr()), ctypes.c_void_p(self.kernel_ptr_sparse.data_ptr()), ctypes.c_void_p(self.kernel_map_sparse.data_ptr()))
