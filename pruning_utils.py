...
def prune_model_custom_fillback_slow(model, mask_dict, conv1=False):
    for name,m in model.named_modules():
        if isinstance(m, nn.Conv2d):
            if (name == 'conv1' and conv1) or (name != 'conv1'):
                mask = mask_dict[name+'.weight_mask']
                ...
                print(norm.shape)
                print(mask.mean())
                mask2 = mask[torch.where(count < threshold)[0]]
                mask2[norm < threshold2] = 0
                mask2[norm >= threshold2] = 1
                mask[torch.where(count < threshold)[0]] = mask2
                print(mask.mean())
