...
def tiny_imagenet_with_trigger_dataloaders(batch_size=64, data_dir = 'datasets/tiny-imagenet-200', dataset=False, split_file=None):
    ...
    train_loader = DataLoader(train_set, batch_size=batch_size - 2, shuffle=True, num_workers=8, pin_memory=True)
    val_loader = DataLoader(val_set, batch_size=batch_size, shuffle=False, num_workers=2, pin_memory=True)
    test_loader = DataLoader(test_set, batch_size=batch_size, shuffle=False, num_workers=2, pin_memory=True)
    trigger_set_loader = getwmloader()
    if dataset:
